...
create function valeTransporte(pCPF varchar(14))
    ...
    begin
		declare valeTransp decimal(5,2);
        declare auxCidade varchar(60);
        declare auxSalario decimal(7,2);
        
        select cidade into auxCidade from tropicanatads042.endereco 
			where Funcionario_cpf = pCPF;
		select salario into auxSalario from tropicanatads042.funcionario
			where cpf = pCPF;
            
        if auxCidade like "Recife" then set valeTransp = 2 * 22 * 4.3;
		else set valeTransp = 2 * 22 * 5.1;
        end if;
		
        set valeTransp = valeTransp - auxSalario * 0.06;
